...
math_tools = {
    ...
    9: ("順列（P）", fpc.perm),
    10: ("組み合わせ（C）", fpc.comb),
    11: ("重複組み合わせ（H）", fpc.h_comb),
    12: ("10進数 → M進数変換", base_change.base_m),
    13: ("M進数 → 10進数変換", base_change.base_10),
    14: ("M進数での四則演算", base_change.base_m_calc),
    15: ("多項式の割り算", poly_div.main),
    16: ("平面上の内分点・中点・外分点を求める", div_point.main),
    17: ("2点間の距離を求める", distance.p2),
    18: ("点と直線の距離を求める", distance.pl),
    19: ('平行な2直線間の距離', distance.ll),
    20: ("三角形の形状・面積を調べる", triangle.main),
    21: ("三角形の重心を求める", triangle.gravity_point),
    22: ("三角比の値を調べる", tri_ratio.main),
    23: ("三角方程式を解く", tri_equation.main),
    24: ("指数方程式・対数方程式を解く", expo_equation.main),
    25: ("積分を行う", integrate.main),
